...
def validate_result(string):
    ...
    sample_text = string.upper().replace('.', '').replace('!', '').replace('?', '').strip()  # Ensure capital format and remove full stop and blank space
...
def main():
    ...
    try:
        while True:  # Run forever
            if keyboard.is_pressed('ENTER'):  # Check if button is pressed
                ser.write(b'#')
